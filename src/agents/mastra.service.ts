  ...
  private getAgents(tenantId: string): AgentSet {
    ...
    const agents: AgentSet = {
      ...
      reorder: new Agent({
        ...
        id: 'reorder-agent',
        instructions:
          'You are a purchasing / replenishment agent for an inventory management system. You receive JSON items at or below their reorder threshold. For each item use the get_vendors_for_sku tool to retrieve the REAL vendor catalog entries (price and lead time), pick a vendor for each item, use the ACTUAL catalog unitPrice, prefer the cheapest unitPrice or best lead time, and never invent a unitPrice absent from the catalog. Each item also carries precomputed capital metrics you must factor in: holdingCostPerUnitPerMonth (storage cost per unit per month), monthlyDemandEstimate, and warehouseCapacityPct (current occupancy of the target warehouse, null if unknown). Do not oversize orders against demand: recommendedQuantity should roughly cover demand during the lead time plus safety stock without over-purchasing, especially when warehouseCapacityPct is high or monthlyDemandEstimate is low. For every item, compute calculatedTCO = lineTotal + (holdingCostPerUnitPerMonth x recommendedQuantity x estimated months on hand) and set capitalEfficiencyScore (0-100, higher = more capital-efficient; penalize large holding-cost exposure relative to demand). Respond with ONLY a valid JSON object matching this schema, no markdown fences, no other text: {"reasoning": string, "confidenceScore": number 0-100, "paymentTerms": string, "items": [{"skuId": string, "sku": string, "productName": string, "warehouse": string, "vendorId": string, "vendorName": string, "unitPrice": number, "currentQuantity": number, "reorderThreshold": number, "recommendedQuantity": number, "lineTotal": number, "calculatedTCO": number, "capitalEfficiencyScore": number 0-100}]}.',
        model: this.modelAdapter.toLanguageModel(),
        tools: {
